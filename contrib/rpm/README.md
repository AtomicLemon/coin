...
The RPM spec file provided here is for BFN-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://BFN.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BFN git repository](https://github.com/BFN/BFN) but are not part of
...
Currently only `Source10` is used, for the example `BFN.conf` file.
...
in the BFN git repository. At present only an SVG version of the BFN
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the BFN
developers so that an official fix to the issue can make it into the next
release.

### Patch0 BFN-0.12.0-libressl.patch
...
needed when 0.12.1 is released, a proper fix is already in the BFN git
master branch.
...
idea to build BFN Core against a static version of that library compiled
...
The `BFN.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `BFN-server` package. At this point, most RPM based
...
The `BFN-server` package should have SELinux support. How to properly do
...
BFN-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/BFN/). He has been
packaging BFN for Fedora at least since 2012.
...
4. I renamed the `BFN` package that contains the Qt GUI to `BFN-core` as
that appears to be how the general population refers to it, in contrast to
`BFN-xt` or `BFN-classic`. I wanted to make sure the general population
