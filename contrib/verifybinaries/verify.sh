...
###   This script attempts to download the signature file SHA256SUMS.asc from BFN.org
...
RCSUBDIR="test/"
BASEDIR="https://BFN.org/bin/"
VERSIONPREFIX="BFN-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'BFN-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BFN Core binary release signing key installed?"
   fi
