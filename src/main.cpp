...
// Copyright (c) 2017-2018 The BFN Core developers
...
#if defined(NDEBUG)
# error "BFN cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BFN Signed Message:\n";
 ...
 * Also removes from the vector of input heigBFN any entries which did not
 * correspond to sequence locked inputs as they do not affect the calculation.
 */
static std::pair<int, int64_t> CalculateSequenceLocks(const CTransaction &tx, int flags, std::vector<int>* prevHeigBFN, const CBlockIndex& block)
{
    assert(prevHeigBFN->size() == tx.vin.size());
    ...
    for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
        ...
        if (txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_DISABLE_FLAG) {
            ...
            (*prevHeigBFN)[txinIndex] = 0;
            continue;
        }

        int nCoinHeight = (*prevHeigBFN)[txinIndex];
...
bool SequenceLocks(const CTransaction &tx, int flags, std::vector<int>* prevHeigBFN, const CBlockIndex& block)
{
    return EvaluateSequenceLocks(block, CalculateSequenceLocks(tx, flags, prevHeigBFN, block));
}
...
bool CheckSequenceLocks(const CTransaction &tx, int flags, LockPoints* lp, bool useExistingLockPoints)
{
    ...
    if (useExistingLockPoints) {
        ...
        std::vector<int> prevheigBFN;
        prevheigBFN.resize(tx.vin.size());
        for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
            const CTxIn& txin = tx.vin[txinIndex];
            CCoins coins;
            ...
            if (coins.nHeight == MEMPOOL_HEIGHT) {
                ...
                prevheigBFN[txinIndex] = tip->nHeight + 1;
            } else {
                prevheigBFN[txinIndex] = coins.nHeight;
            }
        }
        lockPair = CalculateSequenceLocks(tx, flags, &prevheigBFN, index);
        if (lp) {
            ...
            BOOST_FOREACH(int height, prevheigBFN) {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            for (size_t j = 0; j < tx.vin.size(); j++) {
                prevheigBFN[j] = view.AccessCoins(tx.vin[j].prevout.hash)->nHeight;
            }
            ...
            if (!SequenceLocks(tx, nLockTimeFlags, &prevheigBFN, *pindex)) {
    ...
    for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
        ...
        if(nAccValue > nPOWBlockReward && block.vtx[0].vout[i].nValue > 0) {
            ...
            if(metadata[nPaymentRequestsCount].isStr()) {
                ...
                if(block.vtx[0].vout[i].nValue != prequest.nAmount || prequest.fState != CFund::ACCEPTED || proposal.Address != CBFNAddress(address).ToString())
...
//
// Commented - BFN uses now version control
//
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of BFN, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of BFN, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of BFN, please update.";
            fObsolete = true;
        }
...
bool SendMessages(CNode* pto)
{
 ...
 std::string CBlockFileInfo::ToString() const {
     return strprintf("CBlockFileInfo(blocks=%u, size=%u, heigBFN=%u...%u, time=%s...%s)", nBlocks, nSize, nHeightFirst, nHeightLast, DateTimeStrFormat("%Y-%m-%d", nTimeFirst), DateTimeStrFormat("%Y-%m-%d", nTimeLast));
 }
