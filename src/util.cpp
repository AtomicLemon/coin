...
const char * const BFN_CONF_FILENAME = "BFN.conf";
const char * const BFN_PID_FILENAME = "BFN.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BFN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BFN
    // Mac: ~/Library/Application Support/BFN
    // Unix: ~/.BFN
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FLORA";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FLORA";
#else
    // Unix
    return pathRet / ".FLORA";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", BFN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override BFN.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", BFN_PID_FILENAME));
